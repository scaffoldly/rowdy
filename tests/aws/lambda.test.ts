...
describe('aws lambda', () => {
  ...
  describe('lambda function', () => {
    aws('should create default sandbox', async () => {
      const fn = new LambdaFunction('Sandbox', imageService);
      const state = await new Promise<LambdaFunction['State']>((resolve, reject) => {
        fn.observe().subscribe({
          next: (state) => {
            console.log('!!! state updated', state);
          },
          error: reject,
          complete: () => resolve(fn.State),
        });
      });
