    ...
    describe('pullImage', () => {
      describe('aws', () => {
        aws(
          'should pull alpine',
          async () => {
            const { image, imageRef } = await lastValueFrom(rowdy.images.pullImage('alpine'));
        ...
        aws('should pull ubuntu', async () => {
          const { image, imageRef } = await lastValueFrom(rowdy.images.pullImage('ubuntu'));
          expect(image).toEqual('ubuntu');
          expect(imageRef).toMatch(
            /^[0-9]{12}\.dkr\.ecr\.[a-z0-9-]+\.amazonaws\.com\/library\/ubuntu@sha256:[a-f0-9]{64}$/
          );
        });
        aws('should pull busybox', async () => {
          const { image, imageRef } = await lastValueFrom(rowdy.images.pullImage('busybox'));
